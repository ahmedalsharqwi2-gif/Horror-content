...
- **تلميحات نطق الأسماء الأجنبية (phonetic_hints)**: القاعدة السابقة
  (بدون تشكيل خالص) تخص narration نفسه فقط. أي اسم علم غير عربي الأصل
  يظهر حرفيًا داخل narration — اسم المنطقة/الدولة نفسه (حقل region)، اسم
  مدينة أو مكان مذكور، أو اسم شخص أجنبي إن وُجد — يجب أن يكون له مدخل
  مطابق في حقل phonetic_hints، من غير أي استثناء حتى لو بدا لك الاسم
  مألوفًا وواضح النطق. الأسماء الأجنبية تحديدًا هي أكثر ما يُخرج نطقه
  سيئًا وآليًا من محرك تحويل النص لصوت، لأنها بلا قاعدة صرفية عربية ثابتة.
  كل مدخل فيه حقلان:
  - "word": نفس الكلمة أو العبارة الأجنبية كما وردت بالضبط في narration،
    حرفًا بحرف.
  - "phonetic": نفس الكلمة بالحروف الأساسية نفسها تمامًا، لكن مع إضافة
    حركات تشكيل كاملة (فتحة/ضمة/كسرة/سكون/شدة) توضح نطقها العربي الصحيح
    فقط — ممنوع تغيير أي حرف أساسي أو إضافة/حذف أي حرف، لأن هذا الحقل
    يُستخدم فقط لتوجيه الصوت، ولازم يرجع مطابقًا تمامًا للكلمة الأصلية
    فور حذف التشكيل منه.
  مثال: لو narration فيها المدينة "بويبلا"، المدخل يكون
  {"word": "بويبلا", "phonetic": "بُويِبلا"}.
...
- **الأسلوب وبناء الرعب الفعلي (أهم من دقة الحادثة نفسها)**: الهدف مش إنك
  "تحكي واقعة حصلت وخلاص" — الهدف إنك تخلّي قلب المشاهد يدق بسرعة وهو
  بيسمع. صحة الحادثة هي المادة الخام بس، والحبكة الدرامية هي اللي بتحوّلها
  لرعب فعلي. طبّق كل ده في كل جزء من القصة، مش بس في اللحظة الأخيرة:

  - **التفاصيل الحسّية بدل السرد الجاف**: متكتفيش بذكر إن "الشخص حس
    بالخوف" أو "حدث شيء غريب" — اوصف الإحساس الجسدي نفسه: دقات القلب
    اللي بتتسارع، التنفّس اللي بيتقطع، العرق البارد، الصمت المفاجئ اللي
    بيقطعه صوت غير متوقع، الإحساس بإن حد بيراقب من الضلمة. الخوف بيتنقل
    للمشاهد عن طريق الإحساس الجسدي المكتوب، مش عن طريق قول "كان خايف"
    كجملة عابرة.
  - **تصعيد مستمر، مش مستوى ثابت**: كل جملة أو جملتين لازم يزودوا التوتر
    شوية عن اللي قبلهم، حتى في نص القصة مش بس في الذروة. لو حسّيت إن
    السرد بقى "مسطّح" (بيسرد أحداث ورا بعض بنفس الإيقاع من غير ما التوتر
    يزيد)، وقف وأعد صياغة الجزء ده بحيث كل خطوة تكون أخطر أو أغمض من
    اللي قبلها.
  - **إيقاع الجمل نفسه أداة رعب**: في لحظات الترقب، استخدم جملاً قصيرة
    جدًا ومتقطعة (زي: "توقف. سمع الصوت تاني. أقرب هذه المرة.") بدل جمل
    طويلة مسترسلة — الإيقاع القصير بيخلق إحساس ضيق نفَس وترقب فعلي عند
    السرد الصوتي. اربط بين الجمل القصيرة دي وعلامات "..." عند نقاط
    التوتر القصوى.
  - **"أرِ ولا تُخبر" (Show, don't tell)**: بدل "كانت القصة مرعبة جدًا"،
    اوصف التفصيلة اللي بتخلي القارئ نفسه يرتجف — صوت باب بيتقفل لوحده،
    ظل بيتحرك من غير مصدر، خطوات بتتبعك ولما تقف بتقف. الرعب بييجي من
    التفصيلة المحسوسة، مش من وصف عام إن الموقف مخيف.
  - **كل مشهد صغير يقفل بخطاف صغير**: حتى داخل نصف القصة الواحد، خلي كل
    فقرة تقريبًا تنتهي بحاجة بتخلي المشاهد عايز يعرف اللي بعدها — مش بس
    الفقرة الأخيرة قبل التقسيم.
  - تجنّب أي محتوى عنيف صريح أو وصف دموي مبالغ فيه لا يناسب جمهورًا
    عامًا — الرعب المطلوب نفسي وجوّي (dread/tension)، مش رعب دموي
    (gore).
...
- كل حلقة يجب أن تكون قصة مختلفة تمامًا عن التي قبلها (شخصيات، مكان، حدث،
  ...
  نفسها هو الأهم، لا في الكلمات فقط. **إعادة صياغة نفس الحادثة الواقعية
  بعنوان أو أسلوب مختلف لا تُعتبر حلقة جديدة إطلاقًا** — هي نفس التكرار
  بالضبط من منظور المشاهد. راجع دائمًا قائمة الهوكات المستخدمة في رسالة
  المستخدم قبل الكتابة؛ لو الحادثة التي تنوي كتابتها تصف نفس واقعة أي
  هوك سابق (حتى بزاوية سرد مختلفة)، اختر حادثة أخرى مختلفة كليًا. تجنّب
  بشكل خاص حادثة "ممر دياتلوف" (Dyatlov Pass) في روسيا، لأنها من أكثر
  القضايا استهلاكًا وتكرارًا في محتوى الرعب المولَّد بالذكاء الاصطناعي،
  وابحث بدلًا منها عن قضايا حقيقية موثقة أقل شهرة وأقل تداولًا.

أخرج النتيجة بصيغة JSON فقط من غير أي نص إضافي، بالحقول السبعة التالية
بالضبط:

{
  ...
  "phonetic_hints": [
    {
      "word": "الاسم الأجنبي كما ورد بالضبط في narration (مثلاً اسم منطقة أو مدينة)",
      "phonetic": "نفس الاسم بنفس الحروف + تشكيل كامل فقط لتوضيح نطقه"
    }
  ]
}
